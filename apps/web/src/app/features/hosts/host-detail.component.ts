...
@Component({
    ...
    <div class="detail-grid" *ngIf="host() && activeTab() === 'overview'">
      <div class="info-card">
        ...
        <div class="info-rows">
          ...
          <div class="info-row" *ngIf="host()!.site"><span class="info-label">Site Address</span><span class="info-value">{{ formatSiteAddress() }}</span></div>
          ...
          <div class="info-row" *ngIf="host()!.site?.description"><span class="info-label">Site Description</span><span class="info-value">{{ host()!.site!.description }}</span></div>
          <div class="info-row">
          ...
          <div class="info-row" *ngFor="let metric of overviewMetricEntries()">
            <span class="info-label">{{ metricLabel(metric[0]) }}</span>
            <span class="info-value">{{ metric[1] ?? '—' }}</span>
          </div>
          ...
          <div class="info-row" *ngIf="host()!.snmpLastError"><span class="info-label">SNMP Error</span><span class="info-value">{{ host()!.snmpLastError }}</span></div>
          <div class="info-row">
        ...
        <table>
          <thead>
            <tr>
              ...
              <th>MAC</th>
              <th>Oper State</th>
              <th *ngFor="let key of interfaceMetricColumns()">{{ metricLabel(key) }}</th>
            </tr>
          </thead>
          <tbody>
            <tr *ngFor="let item of host()!.snmpInterfaces || []">
              ...
              <td *ngFor="let key of interfaceMetricColumns()">{{ item.metrics?.[key] ?? '—' }}</td>
            </tr>
          </tbody>
        </table>
      </div>
  ...
  styles: [
    `
      ...
      @media (max-width: 900px) { .detail-grid { grid-template-columns: 1fr; } .detail-tabs { flex-wrap: wrap; } }
      ...
      .info-row { display: flex; justify-content: space-between; padding: 12px 20px; border-bottom: 1px solid #f1f5f9; }
