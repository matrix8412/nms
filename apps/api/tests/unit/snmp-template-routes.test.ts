...
describe('SNMP template catalog routes', () => {
  ...
  it('lists SNMP templates for admin requests', async () => {
    prismaMock.snmpOidTemplate.findMany.mockResolvedValueOnce([
      { id: '1', vendor: 'MikroTik', deviceType: null, metricKey: 'softwareVersion', oid: '1.2.3', category: 'OVERVIEW', intervalSec: 1800, enabled: true },
    ]);
  ...
  it('creates a new SNMP template with normalized nullable fields', async () => {
    prismaMock.snmpOidTemplate.create.mockResolvedValueOnce({ id: '2', metricKey: 'hostname', oid: '1.3.6.1.2.1.1.5.0', category: 'OVERVIEW', intervalSec: 1800 });
  ...
  it('accepts custom metric keys', async () => {
    prismaMock.snmpOidTemplate.create.mockResolvedValueOnce({ id: '3', metricKey: 'cpuLoad.1m', oid: '1.3.6.1.4.1.9.2.1.57.0', category: 'OVERVIEW', intervalSec: 300 });
  ...
  it('updates an existing SNMP template', async () => {
    prismaMock.snmpOidTemplate.update.mockResolvedValueOnce({ id: 'row-1', metricKey: 'uptime', oid: '1.3.6.1.2.1.1.3.0', category: 'OVERVIEW', intervalSec: 300 });
